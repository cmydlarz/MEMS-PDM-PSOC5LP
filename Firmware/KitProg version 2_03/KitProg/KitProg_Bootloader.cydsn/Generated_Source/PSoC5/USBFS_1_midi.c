...
const uint8 CYCODE USBFS_1_MIDI_SIZE[] = {
...
volatile uint8 USBFS_1_midiInBuffer[USBFS_1_MIDI_IN_BUFF_SIZE];   /* Input endpoint buffer */
volatile uint8 USBFS_1_midiOutBuffer[USBFS_1_MIDI_OUT_BUFF_SIZE]; /* Output endpoint buffer */
...
#if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF)
    volatile USBFS_1_MIDI_RX_STATUS USBFS_1_MIDI1_Event;          /* MIDI RX status structure */
    volatile uint8 USBFS_1_MIDI1_TxRunStat;                                /* MIDI Output running status */
    volatile uint8 USBFS_1_MIDI1_InqFlags;                                 /* Device inquiry flag */    
    
#if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF)
    volatile USBFS_1_MIDI_RX_STATUS USBFS_1_MIDI2_Event;          /* MIDI RX status structure */
    volatile uint8 USBFS_1_MIDI2_TxRunStat;                                /* MIDI Output running status */
...
void USBFS_1_PrepareInBuffer(uint8 ic, uint8 *srcBuff, uint8 eventLen, uint8 cable)
                                                                 ;
#if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF)
    void USBFS_1_MIDI_Init(void) ;

    uint8 USBFS_1_MIDI1_GetEvent(void) ;
    void USBFS_1_MIDI1_ProcessUsbOut(uint8 *epBuf) ;
    
    #if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF)
        uint8 USBFS_1_MIDI2_GetEvent(void) ;
        void USBFS_1_MIDI2_ProcessUsbOut(uint8 *epBuf) 
                                                    ;
    #endif /* End USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF */
#endif /* End USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF */
...
void USBFS_1_MIDI_EP_Init(void) 
{
    USBFS_1_midiInPointer = 0u;
        ...
        USBFS_1_LoadInEP(USBFS_1_midi_in_ep, (uint8 *)USBFS_1_midiInBuffer,
                                                                                USBFS_1_MIDI_IN_BUFF_SIZE);
        /* Init DMA configurations for OUT EP*/
        USBFS_1_ReadOutEP(USBFS_1_midi_out_ep, (uint8 *)USBFS_1_midiOutBuffer, 
    ...
    if (USBFS_1_GetEPState(USBFS_1_midi_out_ep) == USBFS_1_OUT_BUFFER_FULL)
    {
        outLength = USBFS_1_GetEPCount(USBFS_1_midi_out_ep);
            ...
            outLength = USBFS_1_ReadOutEP(USBFS_1_midi_out_ep, 
                                                   (uint8 *)USBFS_1_midiOutBuffer, outLength);
            #if(USBFS_1_EP_MM == USBFS_1__EP_DMAMANUAL)
                do  /* wait for DMA transfer complete */
                {
                    CyDmaChStatus(USBFS_1_DmaChan[USBFS_1_midi_out_ep], NULL, &dmaState);
        ...
        if (outLength >= USBFS_1_EVENT_LENGTH)
        {
            ...
            while (outPointer < outLength)
            {
                ...
                /* Route USB MIDI to the External connection */
                    ...
                    {
                        USBFS_1_MIDI1_ProcessUsbOut((uint8 *)&USBFS_1_midiOutBuffer[outPointer]);
                    }
                    else if ((USBFS_1_midiOutBuffer[outPointer] & USBFS_1_CABLE_MASK) == 
                             USBFS_1_MIDI_CABLE_01)
                    {
                        #if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF)
                            USBFS_1_MIDI2_ProcessUsbOut((uint8 *)&USBFS_1_midiOutBuffer[outPointer]);
                        #endif /* End USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF */
                    }
                    else
                    {
                ...
                USBFS_1_callbackLocalMidiEvent(
                    ...
                    (uint8 *)&USBFS_1_midiOutBuffer[outPointer + USBFS_1_EVENT_BYTE1]);
...
void USBFS_1_MIDI_IN_EP_Service(void) 
{
    ...
    if ((USBFS_1_midiInPointer != 0u) && 
        (USBFS_1_GetEPState(USBFS_1_midi_in_ep) == USBFS_1_EVENT_PENDING))
    {
        #if(USBFS_1_EP_MM != USBFS_1__EP_DMAAUTO)
            USBFS_1_LoadInEP(USBFS_1_midi_in_ep, (uint8 *)USBFS_1_midiInBuffer,
            ...
            /* rearm IN EP */
            USBFS_1_LoadInEP(USBFS_1_midi_in_ep, NULL, USBFS_1_midiInPointer);
        #endif /* End USBFS_1_EP_MM != USBFS_1__EP_DMAAUTO*/
    ...
    */
        ...
        do 
        {
            if (USBFS_1_midiInPointer <= (USBFS_1_MIDI_IN_BUFF_SIZE - USBFS_1_EVENT_LENGTH))
            {
                ...
                if ((m1 = USBFS_1_MIDI1_GetEvent()) != 0u)
                {
            ...
            if (USBFS_1_midiInPointer <= (USBFS_1_MIDI_IN_BUFF_SIZE - USBFS_1_EVENT_LENGTH))
            {
                ...
                if ((m2 = USBFS_1_MIDI2_GetEvent()) != 0u)
                {
             ...
             (USBFS_1_midiInPointer <= (USBFS_1_MIDI_IN_BUFF_SIZE - USBFS_1_EVENT_LENGTH)));
    #endif /* End USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF */
                                                            ...
                                                            
{
                ...
                (USBFS_1_EP[USBFS_1_midi_in_ep].bufferSize - USBFS_1_EVENT_LENGTH))
    {
        ...
        {   /* only SysEx message is greater than 4 bytes */
            do
            {
                USBFS_1_PrepareInBuffer(USBFS_1_MIDI_SYSEX, midiMsg, 
                                                 USBFS_1_EVENT_BYTE3, cable);
                ic -= USBFS_1_EVENT_BYTE3;
                midiMsg += USBFS_1_EVENT_BYTE3;
                if (USBFS_1_midiInPointer > 
                    (USBFS_1_EP[USBFS_1_midi_in_ep].bufferSize - USBFS_1_EVENT_LENGTH))
                {
                    USBFS_1_MIDI_IN_EP_Service();
    ...
    else if((ic < USBFS_1_EVENT_LENGTH) || (ic == USBFS_1_MIDI_SYSEX))
    {
        ...
        else if (srcBuffZero < USBFS_1_MIDI_SYSEX)
        {
            USBFS_1_midiInBuffer[USBFS_1_midiInPointer] =
                                                    ((srcBuffZero >> 4u) & USBFS_1_CIN_MASK) | cable;
            USBFS_1_midiInPointer++;
        }
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBFS_1_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            ...
            {
                if ((mData & USBFS_1_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(singl byte) */
                {
                    midiReturn = mData;
                }
