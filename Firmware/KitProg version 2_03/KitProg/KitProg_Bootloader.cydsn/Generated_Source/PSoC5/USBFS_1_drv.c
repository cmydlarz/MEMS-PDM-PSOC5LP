...
CY_ISR(USBFS_1_EP_0_ISR)
{
    ...
    if (bRegTemp & USBFS_1_MODE_ACKD)
    {
        ...
        if (bRegTemp & USBFS_1_MODE_SETUP_RCVD)
        {
            ...
            {
                USBFS_1_HandleSetup();
                if(USBFS_1_ep0Mode & USBFS_1_MODE_SETUP_RCVD)
                {
        ...
        if(modifyReg != 0u)
        {
            ...
            if((bRegTemp & USBFS_1_MODE_SETUP_RCVD) == 0)  /* Check if SETUP bit is not set, otherwise exit */
            {
                /* Update the count register */
                bRegTemp = USBFS_1_ep0Toggle | USBFS_1_ep0Count;
                CY_SET_REG8(USBFS_1_EP0_CNT_PTR, bRegTemp);
                if(bRegTemp == CY_GET_REG8(USBFS_1_EP0_CNT_PTR))   /* continue if writing was succesful */
                {
                    do
                    {
...
void USBFS_1_HandleSetup(void) 
{
    ...
    if(requestHandled & USBFS_1_MODE_SETUP_RCVD)
    {
...
*     current packet.
*  USBFS_1_lastPacketSize - remebers the USBFS_ep0Count value for the
*     next packet.
*  USBFS_1_transferByteCount - suma of the previos bytes transferred
...
void USBFS_1_LoadEP0(void) 
{
    ...
    USBFS_1_ep0Count = 0u;
    while ((USBFS_1_currentTD.count > 0u) && (USBFS_1_ep0Count < 8u))
    {
        USBFS_1_EP0_DR0_PTR[USBFS_1_ep0Count] = *USBFS_1_currentTD.pData++;
        USBFS_1_ep0Count++;
        USBFS_1_currentTD.count--;
    }
    /* Support zero-length packet*/
    if( (USBFS_1_ep0Count > 0u) || (USBFS_1_lastPacketSize == 8u) )
    {
    ...
    USBFS_1_lastPacketSize = USBFS_1_ep0Count;
}
...
{
    uint16 xferCount;
    if(USBFS_1_currentTD.count == 0)
    {
        USBFS_1_InitZeroLengthControlTransfer();
    }
    else
    {
...
void USBFS_1_ControlWriteDataStage(void) 
{
    uint8 *pReg = (uint8 *)USBFS_1_EP0_DR0_PTR;

    USBFS_1_ep0Count = (CY_GET_REG8(USBFS_1_EP0_CNT_PTR ) & 0x0Fu) - 2u;

    USBFS_1_transferByteCount += USBFS_1_ep0Count;

    while ((USBFS_1_currentTD.count > 0u) && (USBFS_1_ep0Count > 0u))
    {
        *USBFS_1_currentTD.pData++ = CY_GET_REG8(pReg++);
        USBFS_1_ep0Count--;
        USBFS_1_currentTD.count--;
    }
    /* Update the data toggle */
