...
extern volatile T_USBFS_1_EP_CTL_BLOCK USBFS_1_EP[];
    ...
    extern volatile uint8 USBFS_1_hidProtocol[];
...
void USBFS_1_Init(void) 
{
    ...
    /* ENABLING USBIO PADS IN USB MODE FROM I/O MODE */
        ...
        */
        USBFS_1_PM_USB_CR0_REG &= ~(USBFS_1_PM_USB_CR0_PD_N |USBFS_1_PM_USB_CR0_PD_PULLUP_N);
        ...
        USBFS_1_DM_INP_DIS_REG &= ~USBFS_1_DM_MASK;
        USBFS_1_DP_INP_DIS_REG &= ~USBFS_1_DP_MASK;
    ...
    /* Write WAx */
    CY_SET_REG8(&USBFS_1_ARB_RW1_WA_PTR[0u],     0u);
    CY_SET_REG8(&USBFS_1_ARB_RW1_WA_MSB_PTR[0u], 0u);
    ...
    /* Set the Data Endpoint 8 Interrupt. */
        ...
        CyIntSetVector(USBFS_1_ARB_VECT_NUM,   USBFS_1_ARB_ISR);
...
void USBFS_1_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
        ...
        for (interface = 0u; interface < USBFS_1_MAX_INTERFACES_NUMBER; interface++)
        {
            USBFS_1_hidProtocol[interface] = USBFS_1_PROTOCOL_REPORT;
        }
    #endif /* USBFS_1_ENABLE_HID_CLASS */
...
uint8 USBFS_1_CheckActivity(void) 
{
    ...
    CY_SET_REG8(USBFS_1_CR1_PTR, (r & ~USBFS_1_CR1_BUS_ACTIVITY));
...
uint16 USBFS_1_GetEPCount(uint8 epNumber) 
{
    ...
    if((epNumber > USBFS_1_EP0) && (epNumber < USBFS_1_MAX_EP))
    {
        ...
        count = ((uint16)((CY_GET_REG8(&USBFS_1_SIE_EP1_CNT0_PTR[ri]) & USBFS_1_EPX_CNT0_MASK) << 8u)
                         | CY_GET_REG8(&USBFS_1_SIE_EP1_CNT1_PTR[ri]))
    ...
    *     by the USBFS_1_LoadInEP() or USBFS_1_ReadOutEP() APIs.
    *
    * Parameters:
    ...
    *******************************************************************************/
    void USBFS_1_Stop_DMA(uint8 epNumber) 
    {
        ...
        do
        {
            ...
            {
                CyDmaChDisable(USBFS_1_DmaChan[i]);
                                                                        ...
                                                                        
{
    ...
    {
        ri = ((epNumber - USBFS_1_EP1) << USBFS_1_EPX_CNTX_ADDR_SHIFT);
        p = (uint8 *)&USBFS_1_ARB_RW1_DR_PTR[ri];
        ...
        CY_SET_REG8(&USBFS_1_SIE_EP1_CNT0_PTR[ri], (length >> 8u) | (USBFS_1_EP[epNumber].epToggle));
        CY_SET_REG8(&USBFS_1_SIE_EP1_CNT1_PTR[ri],  length & 0xFFu);
            ...
            USBFS_1_SIE_EP1_CR0_PTR[ri] = USBFS_1_EP[epNumber].epMode;
        #else
            ...
            if((pData != NULL) && (length > 0))
            {
                ...
                CyDmaChDisable(USBFS_1_DmaChan[epNumber]);
                CyDmaTdSetConfiguration(USBFS_1_DmaTd[epNumber], length, DMA_INVALID_TD,\
                                                                                    TD_TERMIN_EN | TD_INC_SRC_ADR);
                CyDmaTdSetAddress(USBFS_1_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Enable the DMA */
                CyDmaChSetInitialTd(USBFS_1_DmaChan[epNumber], USBFS_1_DmaTd[epNumber]);
                CyDmaChEnable(USBFS_1_DmaChan[epNumber], 1);
                /* Generate DMA request */
                USBFS_1_ARB_EP1_CFG_PTR[ri] |= USBFS_1_ARB_EPX_CFG_DMA_REQ;
                USBFS_1_ARB_EP1_CFG_PTR[ri] &= ~USBFS_1_ARB_EPX_CFG_DMA_REQ;
            ...
            if(pData != NULL)
            {
                ...
                CyDmaChDisable(USBFS_1_DmaChan[epNumber]);
                CyDmaTdSetConfiguration(USBFS_1_DmaTd[epNumber], length, USBFS_1_DmaTd[epNumber],\
                                                                                    TD_TERMIN_EN | TD_INC_SRC_ADR);
                CyDmaTdSetAddress(USBFS_1_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Clear Any potential pending DMA requests before starting the DMA channel to transfer data */
                CyDmaClearPendingDrq(USBFS_1_DmaChan[epNumber]);
                /* Enable the DMA */
                CyDmaChSetInitialTd(USBFS_1_DmaChan[epNumber], USBFS_1_DmaTd[epNumber]);
                CyDmaChEnable(USBFS_1_DmaChan[epNumber], 1);
            }
                                                                        ...
                                                                        
{
    ...
    #if(USBFS_1_EP_MM != USBFS_1__EP_DMAAUTO)
        uint16 xferCount;
    #endif /* End USBFS_1_EP_MM != $INSTANCE_NAME`__EP_DMAAUTO */
    ...
    if((epNumber > USBFS_1_EP0) && (epNumber < USBFS_1_MAX_EP) && (pData != NULL))
    {
        ...
        p = (uint8 *)&USBFS_1_ARB_RW1_DR_PTR[ri];
            ...
            CyDmaTdSetAddress(USBFS_1_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
            /* Enable the DMA */
            CyDmaChSetInitialTd(USBFS_1_DmaChan[epNumber], USBFS_1_DmaTd[epNumber]);
            CyDmaChEnable(USBFS_1_DmaChan[epNumber], 1);
...
void USBFS_1_EnableOutEP(uint8 epNumber) 
{
    ...
    if((epNumber > USBFS_1_EP0) && (epNumber < USBFS_1_MAX_EP))
    {
        ...
        USBFS_1_EP[epNumber].apiEpState = USBFS_1_NO_EVENT_PENDING;
        /* Write the Mode register */
        CY_SET_REG8(&USBFS_1_SIE_EP1_CR0_PTR[ri], USBFS_1_EP[epNumber].epMode);
    }
}
...
uint8 USBFS_1_GetEPAckState(uint8 epNumber) 
{
    ...
    {
        ri = ((epNumber - USBFS_1_EP1) << USBFS_1_EPX_CNTX_ADDR_SHIFT);
        cr = CY_GET_REG8(&USBFS_1_SIE_EP1_CR0_PTR[ri]) & USBFS_1_MODE_ACKD;
    }
...
void USBFS_1_SetPowerStatus(uint8 powerStatus) 
{
    ...
    *******************************************************************************/
    ...
    {
        return((CY_GET_REG8(USBFS_1_VBUS_PS_PTR) & USBFS_1_VBUS_MASK) ? 1u : 0u);
    }
