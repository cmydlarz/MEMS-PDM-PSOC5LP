                                                            ...
                                                            `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommRead")`
{
    ...
    if(`$INSTANCE_NAME`_started)
    {
        ...
        {
            `$INSTANCE_NAME`_IsConfigurationChanged();  /* Crear configuration changes state status */
    ...
    {
        if(`$INSTANCE_NAME`_IsConfigurationChanged() != 0) /* Host could send double SET_INTERFACE request or RESET */
        {
            if(`$INSTANCE_NAME`_GetConfiguration() != 0)    /* Init IN endpoints when device configured */
            {
                `$INSTANCE_NAME`_CyBtldrCommReset();
            }
        }    
