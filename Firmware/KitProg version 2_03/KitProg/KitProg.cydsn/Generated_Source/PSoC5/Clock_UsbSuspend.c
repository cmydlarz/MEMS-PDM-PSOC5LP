...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_UsbSuspend_CLKEN |= Clock_UsbSuspend_CLKEN_MASK;
	Clock_UsbSuspend_CLKSTBY |= Clock_UsbSuspend_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_UsbSuspend_CLKEN &= (uint8)(~Clock_UsbSuspend_CLKEN_MASK);
	Clock_UsbSuspend_CLKSTBY &= (uint8)(~Clock_UsbSuspend_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_StopBlock(void) 
{
    if ((Clock_UsbSuspend_CLKEN & Clock_UsbSuspend_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_UsbSuspend__CFG3)
        CLK_DIST_AMASK = Clock_UsbSuspend_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_UsbSuspend_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_UsbSuspend__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_UsbSuspend_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_UsbSuspend_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_UsbSuspend_CLKSTBY &= (uint8)(~Clock_UsbSuspend_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_UsbSuspend_GetSourceRegister();
    uint16 oldDivider = Clock_UsbSuspend_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_UsbSuspend_CLKEN & Clock_UsbSuspend_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_UsbSuspend_DIV_PTR, clkDivider);
                Clock_UsbSuspend_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_UsbSuspend_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_UsbSuspend_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_UsbSuspend_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_UsbSuspend_DIV_PTR, clkDivider);
				Clock_UsbSuspend_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_UsbSuspend_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_UsbSuspend_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_SetModeRegister(uint8 modeBitMask) 
{
    Clock_UsbSuspend_MOD_SRC |= modeBitMask & (uint8)Clock_UsbSuspend_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_UsbSuspend_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_UsbSuspend_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_UsbSuspend_GetModeRegister(void) 
{
    return Clock_UsbSuspend_MOD_SRC & (uint8)(Clock_UsbSuspend_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_UsbSuspend_GetDividerRegister();
    uint8 oldSrc = Clock_UsbSuspend_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_UsbSuspend_MOD_SRC |= CYCLK_SSS;
        Clock_UsbSuspend_MOD_SRC =
            (Clock_UsbSuspend_MOD_SRC & (uint8)(~Clock_UsbSuspend_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_UsbSuspend_GetSourceRegister(void) 
{
    return Clock_UsbSuspend_MOD_SRC & Clock_UsbSuspend_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_UsbSuspend_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_UsbSuspend_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_UsbSuspend_PHASE = clkPhase & Clock_UsbSuspend_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_UsbSuspend_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_UsbSuspend_GetPhaseRegister(void) 
{
    return Clock_UsbSuspend_PHASE & Clock_UsbSuspend_PHASE_MASK;
}
