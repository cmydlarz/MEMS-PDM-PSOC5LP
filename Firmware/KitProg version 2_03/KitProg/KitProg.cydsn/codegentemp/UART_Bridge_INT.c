...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(UART_Bridge_RXISR)
    {
        ...
        readData = UART_Bridge_RXSTATUS_REG;

        if((readData & (UART_Bridge_RX_STS_BREAK | UART_Bridge_RX_STS_PAR_ERROR |
        ...
        while((readData & UART_Bridge_RX_STS_FIFO_NOTEMPTY) != 0u)
        {

            #if (UART_Bridge_RXHW_ADDRESS_ENABLED)
                if(UART_Bridge_rxAddressMode == (uint8)UART_Bridge__B_UART__AM_SW_DETECT_TO_BUFFER)
                {
                    if((readData & UART_Bridge_RX_STS_MRKSPC) != 0u)
                    {
                        if ((readData & UART_Bridge_RX_STS_ADDR_MATCH) != 0u)
                        {
                ...
                {
                    UART_Bridge_rxBuffer[UART_Bridge_rxBufferWrite] = UART_Bridge_RXDATA_REG;
                    increment_pointer = 1u;
                }
            ...
            if( increment_pointer != 0u )
            {
                ...
                if(UART_Bridge_rxBufferWrite >= UART_Bridge_RXBUFFERSIZE)
                {
                ...
                if(UART_Bridge_rxBufferWrite == UART_Bridge_rxBufferRead)
                {
                    ...
                    #endif /* End UART_Bridge_FLOW_CONTROL != 0 */
                }
            }
...
#endif /* End UART_Bridge_RX_ENABLED && (UART_Bridge_RXBUFFERSIZE > UART_Bridge_FIFO_LENGTH) */
    ...
    *******************************************************************************/
    CY_ISR(UART_Bridge_TXISR)
    {
        ...
        {
            /* Check pointer. */
            if(UART_Bridge_txBufferRead >= UART_Bridge_TXBUFFERSIZE)
            {
...
#endif /* End UART_Bridge_TX_ENABLED && (UART_Bridge_TXBUFFERSIZE > UART_Bridge_FIFO_LENGTH) */
